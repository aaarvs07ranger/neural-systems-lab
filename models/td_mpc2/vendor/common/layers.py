...
class Ensemble(nn.Module):
	"""
	Ensemble of modules. De-vectorized for torch 2.2 compatibility (upstream
	vmaps over stacked tensordict params; see ../VENDOR.md). Mathematically
	identical: independent members, outputs stacked on dim 0.
	"""

	def __init__(self, modules, **kwargs):
		super().__init__()
		self._members = nn.ModuleList(modules)
	...
	def forward(self, x, detach_params=False):
		if detach_params:
			outs = [
				torch.func.functional_call(
					m, {k: v.detach() for k, v in m.named_parameters()}, (x,)
				)
				for m in self._members
			]
		else:
			outs = [m(x) for m in self._members]
		return torch.stack(outs, dim=0)

	def __repr__(self):
		return f'Ensemble of {len(self)}x ' + self._repr
